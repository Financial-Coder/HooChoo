...
export default function PostManagementSection() {
    ...
    const [deleteDialogPost, setDeleteDialogPost] = useState<string | null>(null);
    ...
    const handleDeleteClick = (postId: string) => {
        setDeleteDialogPost(postId);
    };

    const handleDeleteConfirm = async () => {
        if (!deleteDialogPost) return;

        setDeleting(deleteDialogPost);
        try {
            await api.delete(`/posts/${deleteDialogPost}`);
            setPosts(posts.filter(p => p.id !== deleteDialogPost));
        } catch (error) {
    ...
    return (
        <>
            ...
            <ConfirmDialog
                isOpen={deleteDialogPost !== null}
                onClose={() => setDeleteDialogPost(null)}
